    ...
    <div className="flex flex-col h-full bg-cream text-near-black">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <div className="space-y-4 pb-8">
           <h3 className="text-[14px] text-[#888] uppercase tracking-[0.08em] font-semibold mb-4">Approval Timeline</h3>
                ...
                <div key={event.id} className="relative">
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="text-[15px] font-semibold text-near-black">{event.approverName}</span>
                      <span className="text-[13px] text-[#888]">{format(new Date(event.timestamp), "MMM d, h:mm a")}</span>
                    </div>
                    <p className="text-[14px] font-medium text-[#666]">{event.action}</p>
                    {event.comment && (
                      <div className="mt-2 bg-white border border-[#E2DDD6] rounded-[6px] p-3 text-[15px] text-[#333] shadow-sm relative">
