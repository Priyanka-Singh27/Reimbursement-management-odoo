  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="text-[28px] font-[400] text-near-black tracking-tight">Team Expenses</h1>
          <p className="text-[15px] text-gray-500 mt-1">Audit and review the broader team's spending log.</p>
        </div>
      </div>

      <div className="bg-white border border-[#E2DDD6] rounded-[14px] shadow-sm overflow-hidden flex flex-col h-[calc(100vh-220px)] min-h-[500px]">
                    ...
                    >
                      <td className="pl-6 pr-4 py-2">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-[#C8C3BB] text-white flex items-center justify-center text-[11px] font-bold">
                            {exp.userId.substring(0, 2).toUpperCase()}
                          </div>
                          <span className="text-[15px] font-medium text-[#1a1a1a]">{exp.userId}</span>
                        </div>
                      </td>
                      <td className="pr-4 py-2 text-[15px] text-[#666] whitespace-nowrap">{format(new Date(exp.date), "MMM d, yyyy")}</td>
                      <td className="pr-4 py-2 text-[15px] text-[#1a1a1a] font-medium truncate max-w-[200px]">
                        {exp.description}
