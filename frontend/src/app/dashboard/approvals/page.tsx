  ...
  if (user?.role === "Employee") {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center text-near-black">
        ...
        <p className="text-[#888] text-[16px]">You do not have approval permissions.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 py-[40px] px-[48px] w-full flex flex-col min-h-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-2">
        <div>
          <h1 className="text-[29px] font-[400] text-near-black tracking-tight">Approval Queue</h1>
          <p className="text-[16px] text-gray-500 mt-1">Review and process team reimbursement requests.</p>
        </div>
        <div className="bg-white px-4 py-2 rounded-[8px] border border-[#E2DDD6] text-[15px] font-medium shadow-sm flex items-center gap-2 text-near-black">
            ...
            >
              <div className="flex-1 p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-[#EFEFEB] text-near-black flex items-center justify-center font-bold text-[19px] shrink-0 border border-[#E2DDD6] overflow-hidden">
                    {expense.userId.substring(0,2).toUpperCase()}
                  </div>
                  <div>
                    <h3 className="text-[19px] font-medium text-near-black mb-0.5">{expense.userId}</h3>
                    <p className="text-[16px] text-[#666] line-clamp-1 max-w-[300px]">{expense.description}</p>
                    <div className="flex gap-2 items-center mt-2.5">
                      <span className="px-2 py-0.5 bg-[#E2DDD6]/50 text-[#666] rounded-[4px] text-[13px] font-medium tracking-wide">
                        {expense.category}
                      </span>
                      <span className="text-[#888] text-[14px]">{format(new Date(expense.date), "MMM dd")}</span>
                      {expense.fraudScore !== undefined && expense.fraudScore >= 50 && (
                         <span className="flex items-center gap-1 px-2 py-0.5 bg-[#FEF3C7] text-[#B5660A] rounded-[4px] text-[13px] font-bold tracking-wide border border-[#D97706]/30">
                ...
                <div className="flex items-center sm:justify-end gap-6 w-full sm:w-auto mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-t-0 border-[#E2DDD6]">
                  <div className="text-left sm:text-right">
                    <p className="text-[21px] font-mono font-[600] text-near-black leading-none mb-1">
                    ...
                    {expense.currency !== company?.defaultCurrency && (
                      <p className="text-[14px] text-[#888] font-mono">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-[#141414] text-white font-medium text-[16px] rounded-[10px] transition-colors hover:bg-[#2a2a2a] whitespace-nowrap active:scale-97"
                  >
                    Review
                  </button>
                </div>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-[720px] max-h-[90vh] translate-x-[-50%] translate-y-[-50%] bg-white shadow-2xl rounded-[16px] outline-none flex flex-col overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
            ...
            <div className="px-8 py-5 border-b border-[#E2DDD6] flex justify-between items-center bg-[#F7F4EF] shrink-0">
              <Dialog.Title className="text-[21px] font-medium text-near-black flex items-center gap-2">
            ...
            <div className="overflow-y-auto flex-1 p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-6">
                <div>
                  <p className="text-[13px] uppercase tracking-wider font-semibold text-[#888] mb-1">Employee</p>
                  <p className="text-[19px] font-medium text-near-black">{reviewingExpense?.userId}</p>
                </div>
                
                <div>
                   <p className="text-[13px] uppercase tracking-wider font-semibold text-[#888] mb-1">Requested Amount</p>
                   <p className="text-[33px] font-mono font-[600] text-near-black leading-none">
                   ...
                   {reviewingExpense?.currency !== company?.defaultCurrency && (
                     <p className="text-[15px] text-[#666] font-mono mt-1">
                ...
                <div className="grid grid-cols-2 gap-4 bg-[#FAFAFA] border border-[#E2DDD6] rounded-[8px] p-4">
                   <div>
                     <p className="text-[13px] uppercase tracking-wider font-semibold text-[#888] mb-1">Date</p>
                     <p className="text-[16px] font-medium text-near-black">{reviewingExpense ? format(new Date(reviewingExpense.date), "MMM do, yyyy") : ""}</p>
                   </div>
                   <div>
                     <p className="text-[13px] uppercase tracking-wider font-semibold text-[#888] mb-1">Category</p>
                     <span className="bg-[#E2DDD6] px-2 py-0.5 rounded-[4px] text-[14px] font-medium text-[#666]">{reviewingExpense?.category}</span>
                   </div>
                </div>

                <div>
                   <p className="text-[13px] uppercase tracking-wider font-semibold text-[#888] mb-1">Description</p>
                   <p className="text-[16px] text-near-black leading-relaxed">{reviewingExpense?.description}</p>
                </div>
                ...
                {reviewingExpense?.fraudScore !== undefined && reviewingExpense.fraudScore >= 50 && (
                   <div className="bg-[#FEF3C7] border border-[#D97706]/40 rounded-[8px] p-4">
                      <div className="flex items-center gap-2 mb-2">
                         ...
                         <span className="text-[15px] font-bold text-[#92400e] uppercase tracking-wider">Signals Found (Score: {reviewingExpense.fraudScore})</span>
                      </div>
                      <ul className="space-y-1">
                         {reviewingExpense.fraudSignals?.map((sig, i) => (
                           <li key={i} className="text-[15px] text-[#B45309] flex items-start gap-2">
              ...
              <div>
                 <p className="text-[13px] uppercase tracking-wider font-semibold text-[#888] mb-2">Attached Receipt</p>
                 {reviewingExpense?.receiptUrl ? (
                   ...
                   <div className="bg-[#FAFAFA] border border-[#E2DDD6] border-dashed rounded-[8px] h-[300px] flex flex-col items-center justify-center text-[#888]">
                     ...
                     <p className="text-[15px] font-medium">No receipt provided</p>
                   </div>
                 )}
              </div>
            ...
            <div className="p-6 pt-0 mt-auto bg-white border-t border-[#E2DDD6] shrink-0">
               <div className="flex flex-col gap-4 pt-6">
                 <div>
                    <label htmlFor="reason" className="block text-[15px] font-semibold text-near-black mb-1">Action Comment (Required for Rejection)</label>
                    <textarea
                      id="reason"
                      rows={2}
                      value={rejectComment}
                      ...
                      className="w-full bg-[#FAFAFA] border border-[#E2DDD6] rounded-[8px] p-3 text-[16px] text-near-black outline-none focus:border-[#141414] transition-all resize-none form-input-focus-ring"
                    />
                 </div>
                 <div className="flex gap-3 justify-end">
                    <button 
                      ...
                      className="px-6 py-2.5 bg-white border border-[#E2DDD6] text-[#9B2335] text-[16px] font-medium rounded-[10px] hover:bg-[#FFF0F0] hover:border-[#F8D7DA] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                    >
                      <X size={16} /> Reject
                    </button>
                    <button 
                      onClick={handleApprove}
                      className="px-6 py-2.5 bg-[#2D6A4F] text-white text-[16px] font-medium rounded-[10px] hover:bg-[#1f4a37] transition-colors flex items-center gap-2"
                    >
