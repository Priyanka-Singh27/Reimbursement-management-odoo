...
export default function DashboardPage() {
  ...
  const renderStatCard = (colorClass: string, label: string, amount: string | number, sub: string, Icon: any) => (
    ...
    >
      <div className="relative z-10 flex flex-col h-full justify-between">
        <h4 className="text-[12px] uppercase tracking-[0.07em] text-[#666] font-semibold mb-2">{label}</h4>
        <div>
          <p className="text-[28px] font-[600] text-near-black font-mono leading-none mb-1">{amount}</p>
          <p className="text-[13px] text-[#888] font-medium">{sub}</p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      {/* Header */}
      <div>
        <h1 className="text-[32px] font-[400] text-[#1a1a1a] tracking-tight">{greeting}</h1>
        <p className="text-[13px] text-gray-500 font-medium mt-1 uppercase tracking-wider">{today}</p>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
             <div className="lg:col-span-7 space-y-4">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-[20px] font-[500] text-[#1a1a1a]">Recent Expenses</h2>
                  <Link href="/dashboard/team-expenses" className="text-[15px] font-medium text-[#666] hover:text-[#1a1a1a] transition-colors">View all &rarr;</Link>
                </div>
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-[#E2DDD6] text-[13px] text-[#888] font-medium uppercase tracking-wider">
                  ...
                  <tbody>
                    {expenses.slice(0, 5).map(exp => (
                      <tr key={exp.id} className="border-b border-[#E2DDD6] hover:bg-white/40 transition-colors h-[52px]">
                        <td className="pr-4 py-2 font-medium text-[15px] text-[#333]">{exp.userId}</td>
                        <td className="pr-4 py-2 text-[15px] text-[#666] hidden sm:table-cell">{format(new Date(exp.date), "MMM d")}</td>
                        <td className="pr-4 py-2 text-[15px] text-[#333] font-medium truncate max-w-[150px]">{exp.description}</td>
                        ...
                        <td className="pr-4 py-2 text-[15px] font-mono font-medium text-[#1a1a1a] text-right">
             ...
             <div className="lg:col-span-5 space-y-4">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-[20px] font-[500] text-[#1a1a1a]">Approval Rules</h2>
                  <Link href="/dashboard/rules" className="text-[15px] font-medium text-[#666] hover:text-[#1a1a1a] transition-colors">Manage &rarr;</Link>
                </div>
                <div className="space-y-3">
                   {rules.map(rule => (
                      <div key={rule.id} className="bg-white border border-[#E2DDD6] rounded-[12px] p-4 flex justify-between items-center">
                         <div>
                            <p className="font-medium text-[#1a1a1a] text-[15px]">{rule.name}</p>
                            <p className="text-[#888] text-[13px] mt-0.5">{rule.steps.length} step(s) logic</p>
                         </div>
                         <span className="bg-[#141414] text-white px-2 py-0.5 rounded-[4px] text-[12px] font-medium tracking-wide">
                            {rule.conditions.length > 0 ? "CONDITIONAL" : "SEQUENTIAL"}
                         </span>
                      </div>
                   ))}
                </div>
                <Link href="/dashboard/rules" className="block w-full py-3 mt-4 text-center border border-[#E2DDD6] border-dashed rounded-[10px] text-[15px] font-medium text-[#666] hover:text-[#1a1a1a] hover:bg-white/50 transition-colors">
                   + New Rule
                </Link>
             </div>
          </div>
        ...
        <motion.div variants={dashboardVariants} initial="hidden" animate="show" className="space-y-10">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-[20px] font-[500] text-[#1a1a1a]">Approval Queue Spotlight</h2>
              <Link href="/dashboard/approvals" className="text-[15px] font-medium text-[#666] hover:text-[#1a1a1a] transition-colors">View all &rarr;</Link>
            </div>
               ...
               {expenses.filter(e => e.status === "Pending" && e.userId !== user.id).slice(0,3).map(exp => (
                  <div key={exp.id} className="bg-white border border-[#E2DDD6] rounded-[12px] p-[20px] flex sm:items-center justify-between hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] transition-shadow duration-300 flex-col sm:flex-row gap-4 sm:gap-0">
                     <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-[#E8C4C4] text-[#9B2335] flex items-center justify-center font-bold text-[15px] shrink-0">
                           {exp.userId.substring(0,2).toUpperCase()}
                        </div>
                        <div>
                           <p className="font-medium text-[#1a1a1a] text-[15px]">{exp.userId}</p>
                           <p className="text-[#666] text-[15px] truncate max-w-[200px]">{exp.description}</p>
                        </div>
                     </div>
                     
                     <div className="flex items-center gap-6">
                        <div className="text-right">
                           <p className="font-mono text-[20px] font-[600] text-[#1a1a1a]">{company?.defaultCurrency} {exp.convertedAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                           <p className="text-[#888] text-[13px] font-medium">{format(new Date(exp.date), "MMM d, yyyy")}</p>
                        </div>
                        <Link href="/dashboard/approvals" className="bg-[#141414] text-white px-4 py-2 rounded-[10px] text-[15px] font-medium hover:bg-[#2a2a2a] transition-transform active:scale-97">
                           Review
                        </Link>
                     </div>
                  </div>
      ...
      {user.role === "Employee" && (
        <motion.div variants={dashboardVariants} initial="hidden" animate="show" className="space-y-10">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-[20px] font-[500] text-[#1a1a1a]">Recent Expenses</h2>
              <Link href="/dashboard/my-expenses" className="text-[15px] font-medium text-[#666] hover:text-[#1a1a1a] transition-colors">View all &rarr;</Link>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[600px]">
                ...
                <tbody>
                  {myExpenses.slice(0, 5).map(exp => (
                    <tr key={exp.id} className="border-b border-[#E2DDD6] hover:bg-white/40 transition-colors h-[52px]">
                      <td className="pr-4 py-2 text-[15px] text-[#666] whitespace-nowrap">{format(new Date(exp.date), "MMM d, yyyy")}</td>
                      <td className="pr-4 py-2 text-[15px] text-[#333] font-medium truncate max-w-[250px]">{exp.description}</td>
                      <td className="pr-4 py-2">
                        <span className="bg-[#E2DDD6] px-2 py-0.5 rounded-[4px] text-[12px] font-medium text-[#666]">{exp.category}</span>
                      </td>
                      <td className="pr-4 py-2 text-[15px] font-mono font-medium text-[#1a1a1a] text-right whitespace-nowrap">
              ...
              {myExpenses.length === 0 && (
                 <div className="text-center py-16">
                    ...
                    <h3 className="text-[20px] font-medium text-[#1a1a1a] mb-1">No expenses yet</h3>
                    <p className="text-[15px] text-[#666] mb-6">You haven't submitted any expenses yet.</p>
                    <Link href="/dashboard/submit" className="bg-[#141414] text-white px-5 py-2.5 rounded-[10px] text-[15px] font-medium hover:bg-[#2a2a2a] transition-transform active:scale-97">
